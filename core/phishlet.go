...
func (p *Phishlet) LoadFromFile(site string, path string, customParams *map[string]string) error {
	...
	if !p.isVersionHigherEqual(&p.Version, "2.2.0") {
		return fmt.Errorf("this phishlet is incompatible with current version of exodus.\nplease do the following modifications to update it:\n\n" +
			...
			"you can buy the phishlet development mastery course on clirpa.com or you can buy a work phishlet from clirpa.com or @eviltwist)")
	}
	if !p.isVersionHigherEqual(&p.Version, "2.3.0") {
		return fmt.Errorf("this phishlet is incompatible with exodus.\nplease do the following modifications to update it:\n\n" +
