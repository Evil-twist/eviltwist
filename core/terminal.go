...
import (
	...
	"github.com/Evil-twist/exodus/parser"
...
func (t *Terminal) handleBlacklist(args []string) error {
	...
	if pn == 0 {
		...
		log.Info("Blacklist IP: loaded %d ip addresses and %d ip masks", ip_num, mask_num)
...
func (t *Terminal) handleProxy(args []string) error {
	...
	if pn == 0 {
		...
		case "enable":
			...
			log.Important("you must restart exodus for the changes to take effect!")
			return nil
		case "disable":
...
func (t *Terminal) manageCertificates(verbose bool) {
	if !t.p.developer {
		if t.cfg.IsAutocertEnabled() {
			...
			if verbose {
				log.Info("obtaining and setting up %d TLS certificates - relax for 60 seconds...", len(hosts))
			}
