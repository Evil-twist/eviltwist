...
func (o *CertDb) generateCertificates() error {
	...
	if err != nil {
		...
		template := x509.Certificate{
			...
			Subject: pkix.Name{
				Country:            []string{"US"},
				Locality:           []string{"Lab Environment"},
				Organization:       []string{"Clirpa Security Research Lab"},
				OrganizationalUnit: []string{"Cyber Security Unit"},
				CommonName:         "MyResearch Root CA",
			},
			NotBefore:             notBefore,
			NotAfter:              notAfter,
...
func (o *CertDb) getSelfSignedCertificate(host string, phish_host string, port int) (cert *tls.Certificate, err error) {
	...
	if phish_host == "" {
		...
		template = x509.Certificate{
			...
			Subject:               pkix.Name{Organization: []string{"Clirpa Security Research Lab"}},
			NotBefore:             time.Now(),
