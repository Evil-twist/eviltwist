...
import (
	...
	"github.com/Evil-twist/exodus/core"
...
func showAd() {
	...
	message := fmt.Sprintf("%s: %s %s", lred.Sprint("Telegram Usernames"), lyellow.Sprint("https://t.me/eviltwist/"), white.Sprint("(Site: clirpa.com)"))
	log.Info("%s", message)
}
...
func main() {
	...
	if *phishlets_dir == "" {
		*phishlets_dir = joinPath(exe_dir, "./phishlets")
		if _, err := os.Stat(*phishlets_dir); os.IsNotExist(err) {
			*phishlets_dir = "/usr/share/exodus/phishlets/"
			if _, err := os.Stat(*phishlets_dir); os.IsNotExist(err) {
				log.Fatal("you need to provide the path to directory where your phishlets are stored: ./exodus -p <phishlets_path>")
				return
			}
		}
	}
	if *redirectors_dir == "" {
		*redirectors_dir = joinPath(exe_dir, "./redirectors")
		if _, err := os.Stat(*redirectors_dir); os.IsNotExist(err) {
			*redirectors_dir = "/usr/share/exodus/redirectors/"
			if _, err := os.Stat(*redirectors_dir); os.IsNotExist(err) {
				*redirectors_dir = joinPath(exe_dir, "./redirectors")
			}
		}
	...
	if *cfg_dir == "" {
		...
		*cfg_dir = filepath.Join(usr.HomeDir, ".exodus")
	}
